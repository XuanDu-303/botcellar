...
type AvailableColor = ColorState["availableColors"][number];
  ...
  persist(
    ...
    }),
    {
      ...
      migrate: (persistedState) => {
        const state = persistedState as ColorState;
        const mergedAvailableColors = mergeAvailableColors(
        ...
        return {
          ...state,
          availableColors: mergedAvailableColors,
          defaultColor: state.defaultColor ?? availableColors[0].name,
        };
      },
    }
  )
...
function mergeAvailableColors(
  persisted: AvailableColor[],
  latest: AvailableColor[]
): AvailableColor[] {
  const colorMap = new Map<string, AvailableColor>();

  [...persisted, ...latest].forEach((color) => {

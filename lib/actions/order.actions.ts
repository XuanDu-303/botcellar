  ...
  const result = await Order.aggregate([
    ...
    },
    {
      $project: {
        ...
        totalSales: 1,// preserve value (1)
      },
    },
    { $sort: { date: 1 } },
  ])
  //example result: [
  //   { date: '2025/4/25', totalSales: 1200 },
  //   { date: '2025/4/26', totalSales: 1500 },
  //   ...
  // ]
  return result
}
...
async function getTopSalesCategories(date: DateRange, limit = 5) {
  const result = await Order.aggregate([
    ...
    // Step 1: Unwind orderItems array
    { $unwind: '$items' },// tách từng item trong items thành một order riêng cùng id
    // Step 2: Group by productId to calculate total sales per product
    {
      $group: {
        _id: '$items.category',
  ...
  // [
  //   { "_id": "Watches", "totalSales": 320 },
  //   { "_id": "Smartphones", "totalSales": 290 },
  //   ...
  // ]
  return result
}
...
export default function ChatbotWidget() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { from: "bot", text: "⚠️ Failed to connect to the chatbot." },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="fixed bottom-6 left-3 z-50">
      ...
      {!isOpen ? (
        ...
        <div className="w-86 h-90 bg-popover border rounded-md shadow-2xl flex flex-col overflow-hidden">
          {/* Header */}
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-4 bg-background text-sm custom-scrollbar">
            ...
            {isLoading && (
              <div
                className={`flex ml-2 space-x-1 justify-start items-center bg-transparent`}
              >
